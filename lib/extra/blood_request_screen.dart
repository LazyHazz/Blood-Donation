    ...
    if (_requestFormKey.currentState!.validate()) {
      ...
      if (user != null) {
        try {
          ...
          // Debug: Print the number of active donors found
          print('Number of active donors found: ${donorSnapshot.docs.length}');

          // Check if donors were found
          if (donorSnapshot.docs.isEmpty) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('No active donors found for $selectedBloodGroup blood group.')),
            );
          }

          // Extract donor information
          List<Map<String, dynamic>> donors = donorSnapshot.docs.map((doc) {
            var data = doc.data() as Map<String, dynamic>;
            print('Donor: ${data['name']}'); // Debug: Print donor details
            return data;
          }).toList();
